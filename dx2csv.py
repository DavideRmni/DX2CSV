...
class DXConverterGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("DX Spectra Converter")
    ...
    def setup_ui(self):
        ...
        # Title
        title_label = ttk.Label(main_frame, text="DX Spectra File Converter", 
                               font=('Arial', 16, 'bold'))
        ...
        # Initial log message
        self.log("Welcome to DX Spectra Converter!")
        ...
        self.log("")
        self.log("1. Select a directory containing .dx files")
    ...
    def select_directory(self):
        ...
        directory = filedialog.askdirectory(title="Select Directory with .dx files")
        if directory:
            self.selected_directory.set(directory)
            self.scan_directory(directory)
            
    def scan_directory(self, directory):
        """Scan directory for .dx files"""
        self.log(f"Scanning directory: {directory}")
        
        try:
            path = Path(directory)
            self.dx_files = list(path.glob('*.dx'))
            ...
            if self.dx_files:
                self.log(f"Found {len(self.dx_files)} .dx files:")
                
                # Create checkboxes for each file
                for i, file_path in enumerate(self.dx_files):
                ...
                self.log("No .dx files found in selected directory")
                self.files_count_label.config(text="No .dx files found")
    ...
    def parse_dx_file(self, file_path):
        """Parse a .dx file and extract metadata and spectral data"""
        ...
        metadata_fields = {
            ...
            'INSTRUMENT PARAMETERS': 'Instrument_Parameters',
            'DELTAX': 'DeltaX',
            'XLABEL': 'XLabel',
            'YLABEL': 'YLabel',
            'XUNITS': 'XUnits',
        ...
        for field, key in metadata_fields.items():
            pattern = f"##\\s*{field}\\s*=\\s*(.+)"
        ...
        # Find data section
        xydata_match = re.search(r'##XYDATA=.*?\n(.*?)##END=', content, re.DOTALL)
        if xydata_match:
            data_section = xydata_match.group(1)
    ...
    def convert_files(self, selected_files):
        ...
        try:
            ...
            for i, file_path in enumerate(selected_files):
                ...
                try:
                    metadata, x_data, y_data = self.parse_dx_file(file_path)
